...
def playGame():
	...
	while (gameOver(dealersPoints, myPoints,name, myCash) !=True):
			...
			test = playRound(dealersPoints, myPoints,0,currentBet)
			# print "test: ", test
...
def playRound(dealersPoints, myPoints,bet,myBet):
	...
	if dealersCard>myCard:
		...
		GotoWar(dealersPoints,myPoints,myBet,myCash,currentBet)
	return {"myPoints": myPoints, "dealersPoints": dealersPoints}

def IsthereaWinner (dealersPoints, myPoints, name):
	print" 	"
	print "Dealer's Points:", dealersPoints, "	", name,"'s Points:", myPoints
...
def gameOver(dealersPoints, myPoints, name, myCash):
	...
	elif dealersPoints ==10 or myCash ==0:
...
def getBet(myCash):
	...
	if myBet %5 ==0 and myBet<=myCash:
		...
		return getBet(myCash)
	
def GotoWar(dealersPoints,myPoints,currentBet,myCash,myBet):
	...
	if myCash>=currentBet*2: 
		currentBet+=currentBet
		myCash-=currentBet
		print "									"
		print "Now your new bet is", currentBet
		print "Discarding cards..."
		for x in range(1,4):
			print x	
	# do countdown and pick cards and winner
		playRound(dealersPoints, myPoints, currentBet,myBet)
